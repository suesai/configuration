...
local function restore_state(window)
	local workspace_name = window:active_workspace()
	...
	if not workspace_data then
		window:toast_notification("WezTerm", "Workspace state file not found for workspace: " .. workspace_name,
			nil, 4000)
		return
	end

	if recreate_workspace(window, workspace_data) then
		window:toast_notification("WezTerm", "Workspace state loaded for workspace: " .. workspace_name, nil,
			4000)
	else
...
local function load_state(window)
	-- TODO: Implement
...
local function save_state(window)
	...
	local file_path = wezterm.home_dir ..
	"/.config/wezterm/wezterm-session-manager/wezterm_state_" .. data.name .. ".json"
...
session_manager.setup = function()
